...
from miniworld.envs import PutNext
from miniworld.entity import Box, TextFrame

from minicrawl.minicrawl import MiniCrawlEnvV2


class BossStageEnv(MiniCrawlEnvV2):
...
class PutNextBossStageEnv(BossStageEnv, PutNext):
    def __init__(self, room_size=12, render_mode="human", view="agent", max_episode_steps=2000,  **kwargs):
        self.max_episode_steps = max_episode_steps
        BossStageEnv.__init__(self, room_size=room_size, render_mode=render_mode, view=view,  **kwargs)
    ...
    def reset(
        ...
        return BossStageEnv.reset(self, seed=seed, options=options)
    ...
    def _gen_world(self):
        ...
        red = self.place_entity(Box(color="red"), pos=np.array([1, 0, self.room_size - 1]))
        green = self.place_entity(Box(color="green"), pos=np.array([self.room_size - 1, 0, 1]))
        yellow = self.place_entity(Box(color="yellow"), pos=np.array([self.room_size - 1, 0, self.room_size - 1]))
        self.putnext_cubes = {
            "blue": blue,
            "red": red,
            "green": green,
