...
from utils import create_traj_metadata
...
class MiniCrawlDataset(Dataset):
    def __init__(self, data_path, history_length=4, use_all=True, max_steps=10000):
        super().__init__()
        ...
        self.traj_metadata = np.genfromtxt(os.path.join(data_path, "trajectories_lengths.csv"), delimiter=",", dtype=object)
        for i in range(self.traj_metadata.shape[0]):
            self.traj_metadata[i, 0] = self.traj_metadata[i, 0].decode('UTF-8')
            self.traj_metadata[i, 1] = int(self.traj_metadata[i, 1].decode('UTF-8'))
    ...
    def __getitem__(self, item):
        ...
        # Load data
        traj = np.load(os.path.join(self.data_path, f"{self.traj_metadata[traj_idx, 0]}.npz"))
        # Build observation
...
def train_bc(agent, env, data_loader, epochs, args, device="cuda", num_eval_games=30):
    ...
    for e in range(epochs):
        ...
        if success > max_success:
            ...
            save_stable_model(Path(f"benchmarks/models/bc/"), agent.policy, "best_model.zip")
            torch.save(agent.policy.state_dict(), f"benchmarks/models/bc/best_model.pth")
            max_success = success
...
def main(args):
    if args.headless:
        pyglet.options["headless"] = True
    ds = MiniCrawlDataset(f"{args.trajectories_base_path}", history_length=args.history_length)
